function GenerateInspectRequestTable() {
    var requests = RetrieveLookupRequests();

    var html = '<table id="table_requests" class="display" style="width:100%">';
    html += '       <thead class="table-rule-header">'
    html += '           <tr>'
    html += '               <th>Request ID</th>'
    html += '               <th>Request</th>'
    html += '               <th style="width:15%">Date Recorded</th>'
    html += '               <th style="text-align:center;width:15%">Response</th>'
    html += '               <th>Name</th>'
    html += '               <th>State</th>'
    html += '           </tr>'
    html += '       </thead >'
    ...
    for (var i = 0; i < requests.length; i++) {
        ...
        html += '               <td colspan="1" class="' + requests[i].RequestId + '"><div class="readonly-component" id="requestId_' + requests[i].RequestId +'"></div></td>'
        html += '               <td></td>'
        html += '               <td></td>'
        html += '               <td>' + requests[i].ReferenceId +'</td>'
        html += '               <td>' + requests[i].Response+'</td>'
        html += '           </tr>';
        
    }
    html += '        </tbody>'
    html += '        <input type="hidden" id="selected_request_id"/>';
    html += '</table >'

    $('#requests_table_container').empty().append(html);

    //var defaultFilters = GetDefaultQueryBuilderFilters();

    //for (var i = 0; i < requests.length; i++) {
    //    $('#requestId_' + requests[i].RequestId).queryBuilder({
    //        plugins: {
    //            'chosen-selectpicker': {
    ...
    //    $('#requestId_' + requests[i].RequestId).queryBuilder('setRules', JSON.parse(requests[i].RequestQuery));
    ...
    var table = $('#table_requests').DataTable({
        ...
        rowGroup: {
            ...
            startRender: function (rows, group) {
                ...
                var groupDetails = requests.filter(c => c.RequestId == group);

                var status_icon = (groupDetails[0].Response === 'GREEN')
                    ? '<i class="fa-solid fa-circle" style="color:#198754; font-size:1.4em"></i>'
                    : '<i class="fa-solid fa-circle" style="color:#dc3545; font-size:1.4em"></i>';

                return $('<tr/>')
                    .append(
                        '<td>' + arrow_icon + groupDetails[0].ReferenceId + '</td>'+
                        '<td>' + groupDetails[0].FormatedDateRecorded + '</td>' +
    ...
    $('#table_requests tbody').on('click', 'tr.dtrg-group', function () {
    ...
    $('#ExpandAllRows').on('click', function () {
        ...
        var requestRows = $('#table_requests tbody .dtrg-group');
        ShowOverlayLoader();
...
function RenderRequestQuery(requestId) {

    if (!$('#requestId_' + requestId).hasClass('query-builder')) {
        var requestBodyContent = RetrieveRequestDetails(requestId);

        $('#requestId_' + requestId).queryBuilder({
            plugins: {
                'chosen-selectpicker': {
                    search_contains: true,
                    width: '10rem'
        ...
        $('#requestId_' + requestId).queryBuilder('setRules', JSON.parse(requestBodyContent));
    }
    var filterCollection = '.readonly-component .rules-group-container .rules-group-body .rules-list .rule-container .rule-filter-container select';
    var operatorCollection = '.readonly-component .rules-group-container .rules-group-body .rules-list .rule-container .rule-operator-container select';
    var valueCollection = '.readonly-component .rules-group-container .rules-group-body .rules-list .rule-container .rule-value-container select';
...
function RetrieveLookupRequests() {
    var requests;

    $.ajax({
        async: false,
        type: "POST",
        url: "/Inspectr/RetrieveLookupRequests",
        success: function(response) {
            requests = JSON.parse(response.result);
        },
...
function RetrieveRequestDetails(id) {
    var data;

    $.ajax({
        async: false,
        type: "POST",
        url: "/Inspectr/RetrieveRequestDetails",
        data: {
            requestId: id
        },
        beforeSend: function () {
            $('#requestId_' + id).LoadingOverlay("show", {
                text: "Loading...",
                textColor: "#131313",
                imageColor: "#3e4b5b",
            });
        ...
        complete: function() {
            $('#requestId_' + id).LoadingOverlay("hide");
        }
    });
